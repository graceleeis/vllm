...
# Compiler flags.
# Avoid debugging symbols to reduce the package size.
...
def get_nvcc_cuda_version(cuda_dir: str) -> Version:
    ...
    """
    nvcc_output = subprocess.check_output([cuda_dir + "/hip/bin/hipcc", "--version"], universal_newlines=True)
...
#             raise ValueError(f"Unsupported CUDA arch ({arch}). " f"Valid CUDA arch strings are: {valid_arch_strs}.")
...
#         # instead of 8.9.
#         warnings.warn(
#             "CUDA 11.8 or higher is required for compute capability 8.9. " "Targeting compute capability 8.0 instead."
#         )
...
cache_extension = CUDAExtension(
    name="vllm.cache_ops",
    sources=["csrc/amd_support/cache.cpp", "csrc/amd_support/cache_kernels.hip"],
    extra_compile_args={
        "cxx": CXX_FLAGS,
        "hipcc": HIPCC_FLAGS,
    },
...
# Misc. CUDA utils.
rocm_utils_extension = CUDAExtension(
    name="vllm.rocm_utils",
    sources=["csrc/amd_support/rocm_utils.cpp", "csrc/amd_support/rocm_utils_kernels.hip"],
    extra_compile_args={
        "cxx": CXX_FLAGS,
        "hipcc": HIPCC_FLAGS,
    },
)
ext_modules.append(rocm_utils_extension)
